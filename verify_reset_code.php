...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['resend_code'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            $resetCodeFromDB = $user['reset_code'];
            ...
            if ($enteredCode == $resetCodeFromDB && time() < $resetExpiry) {
                ...
                if (time() >= $resetExpiry) {
                    $stmt = $conn->prepare("UPDATE users SET reset_code = NULL, reset_expiry = NULL WHERE email = ?");
                    $stmt->bind_param("s", $email);
                    $stmt->execute();
                    $stmt->close();
